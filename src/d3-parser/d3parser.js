module.exports = {
  ...
  parseD3: function (str) {
    ...
    const D3methods = this.returnD3methods();
    ...
    for (let i = 0; i < str.length; i += 1) {
      ...
      substr += str[i];
      if (i === str.length - 1) cache.push(substr);
    }
    // const arr = [];
    // Object.keys(cache).forEach(key => {
    //   if (typeof cache[key] === 'object' && cache[key].args && cache[key].args[0] !== '') {
    //     cache[key].id = key;
    //     //cache[key].name = ;
    //     cache[key].type = 'text';
    //     //cache[key].value = ;
    //     if (cache[key].args.length === 2) {
    //       cache[key].name = cache[key].args[0];
    //       cache[key].value = cache[key].args[1];
    //     }
    //     if (cache[key].args.length === 1) {
    //       cache[key].name = '';
    //       cache[key].value = cache[key].args[0];
    //     }
    //   }
    //   arr.push(cache[key]);
    // });
...
// export {returnD3methods, parseD3, reCode};